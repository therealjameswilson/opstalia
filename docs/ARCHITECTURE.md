...
flowchart LR
    ...
    W <-->|prefix-only GET probe/cancel, bounded full streams;<br/>no redirects| NP
...
- starts a no-redirect full `GET`, validates its response type and declared size, reads only the five-byte `%PDF-` prefix, and cancels the admission body immediately; a reported length above 100 MiB is rejected, while an absent Worker-visible length or validator is permitted; this signature-checked GET is authoritative so method-specific HEAD/GET metadata variance cannot create a false source-change result;
- creates a two-hour HMAC-SHA-256 token signed with `RATE_LIMIT_SALT`; the token signs the source ID, NAID, canonical record and PDF URLs, GET-declared length, any available official object SHA-256 metadata, ETag/last-modified validators, and expiration together to prevent tampering, not to establish the archival association;
...
- validates upstream status and content type, requires any repeated declared length to match, and prefers a mutually available official object SHA-256 value before falling back to same-method ETag or Last-Modified consistency checks;
...
- when the researcher opens a packet, the acknowledged NAID, canonical NARA record URL, and canonical presidential-library PDF URL sent to the Worker; the Worker starts a full `GET`, reads only the five-byte signature prefix, and cancels that admission response;
