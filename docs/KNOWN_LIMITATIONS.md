...
Admission requires an accepted PDF content type and a valid `%PDF-` signature on the exact approved NARA path; redirects are rejected. The Worker starts a full `GET`, reads only the five-byte signature prefix, and cancels that admission response. It pins the GET-declared length and available official checksum or response validators rather than comparing method-specific HEAD/GET metadata. NARA or Cloudflare may omit a Worker-visible length or validator. A reported size above 100 MiB is rejected, and the later content stream is terminated if it crosses the hard 100 MiB limit. A NARA outage, changed path, inconsistent same-method headers, a malformed PDF, or a browser/relay timeout can prevent access even when the file opens through ordinary navigation.
...
The deterministic scan streams only text already embedded in the PDF. It retains no more than 50,000 characters per page, 32 Mi characters across one scan, or 5,000 pages; reaching a ceiling leaves later or truncated pages for manual review and records that the scan was limited. It suppresses all embedded text from a page when PDF.js reports one or more annotations, because an annotation can visually cover text that remains in the file. It does not perform OCR, handwriting recognition, image understanding, or AI analysis. Image-only pages, annotated pages, broken character maps, unusual encodings, columns, stamps, marginalia, and poor source text can yield empty, scrambled, or misleading output. Researchers must inspect page images and define or correct boundaries manually.
...
Beta derivative export downloads one fresh complete admitted source per single or batch operation under a separate three-requests-per-minute rate scope. A cancellable `pdf-lib` Web Worker computes SHA-256 over that copy and refuses export before parsing unless it matches the source hash computed during opening; it is terminated after five minutes. The worker preflights every selected page and refuses the whole operation when any selected page has a non-empty annotation array. On annotation-free pages it rebuilds the reviewed range and removes `/AA` additional-action dictionaries and empty annotation arrays. A batch can contain at most 200 derivatives and 5,000 selected page copies with at most 200 MiB of output. Encrypted, malformed, very complex, or memory-intensive files can fail. Sources above 100 MiB are unsupported by the Packet Lab, including viewing, text search, manifests, and derivative generation.

A derivative changes the file structure and metadata, removes page actions and empty annotation arrays from eligible pages, and is intentionally not byte-identical. Opstalia blocks pages with actual annotations; it does not flatten or remove them. A derivative is not an official source file, certified copy, preservation object, authenticity determination, or new agency release. A page range does not prove that it is an official standalone document, that attachments are complete, that pagination is original, or that the material was released in full. Preserve and cite the official PDF and the separately researcher-supplied Catalog locator; first confirm their association on the Catalog page. Official source records and agency determinations control.

Saved packet registers are browser-local manifests only. They can contain locators, any available source validators, received size, browser-computed source SHA-256, page, scan, and annotation counts, researcher decisions and notes, and structured derivative receipts tied to exact register snapshots. Opstalia does not persist source PDF bytes, rendered pages, thumbnails, embedded page text, ZIP contents, or transport tokens. Reopening requires a new live session and a new full-source transfer. The saved source hash and reviewed decisions are retained only when actual received length and newly computed SHA-256 agree; otherwise every non-rejected decision becomes a proposal requiring re-review and prior derivative receipts are cleared, while an earlier rejection remains recorded. A matching hash proves only that the received byte sequence matches the saved one, not that the file is authentic, complete, or correctly associated with the supplied Catalog record.

When a packet register saved before version 1.3 is reopened, Opstalia cannot prove that its earlier deterministic scan excluded text from annotation-bearing pages. It therefore removes legacy pattern-generated suggestions, clears earlier derivative receipts, resets retained non-rejected researcher-defined items to proposed status, and requires a new annotation-aware scan and human review. This migration changes only Opstalia's browser-local manifest; it does not alter or delete an official source.
