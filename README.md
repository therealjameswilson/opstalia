...
- open an approved NARA Catalog presidential-library PDF in the beta PDF Packet Lab, safety-check its existing text layer, and record reviewed page ranges or described-only items;
- batch-export selected confirmed ranges as a checksummed research ZIP after an explainable gap, overlap, duplicate, and bounds preflight;
...
The Worker validates and canonicalizes the submitted values, starts one no-redirect `GET`, validates its content type and declared size, reads only the five-byte `%PDF-` signature prefix, and cancels that admission response. A reported length greater than 100 MiB is rejected, but NARA or Cloudflare may omit a Worker-visible length or validator. The signature-checked GET is the authoritative admission representation; a method-specific `HEAD` response is not used as evidence that the file changed. The returned two-hour HMAC-signed session binds the NAID, record URL, PDF URL, GET-declared length, any available official object checksum or response validators, and expiration against tampering; it is not evidence that the record and PDF are associated.

Opening the packet starts a new full-file request. The Worker passes that response through once with `no-store` and terminates the stream if more than 100 MiB arrives. It does not buffer the file into application storage. The browser holds the completed source transiently in memory, verifies its PDF signature, records the received byte length, and computes SHA-256 before PDF.js opens it. A derivative export obtains a fresh signed session and requests one complete source copy under a separate rate scope. A batch uses that one copy for every selected range. The isolated processor verifies SHA-256 before parsing and refuses the entire export if the bytes differ from the reviewed source.

PDF.js runs locally over the fully downloaded in-memory bytes; it performs page access and extraction without additional source requests. XFA and script evaluation are disabled. Annotation appearances remain visible in the inert canvas, while the separate embedded-text view and deterministic scan suppress all text for an annotation-bearing page. The scan uses only safety-checked text already embedded in the official PDF—there is no OCR and no AI service—and stops at 50,000 characters per page, 32 Mi characters in memory across the scan, or 5,000 pages. Later, suppressed, or truncated pages remain available for manual visual review. It proposes possible memcon, telcon, memorandum, end-marker, and withdrawal-sheet boundaries. Every proposal is editable and reviewable, and rejected proposals remain visible in the register.
...
Packet registers and researcher decisions can be saved to IndexedDB. Opstalia stores only the manifest—source locators and any available validators, received byte length, source SHA-256, page count, scan and annotation-page counts, reviewed ranges, described-only entries, notes, and structured derivative receipts. It does not persist PDF bytes, rendered pages, thumbnails, ZIP contents, session tokens, or extracted page text. Reopening downloads the current official source again and recomputes SHA-256. Opstalia preserves reviewed decisions only when both received byte length and SHA-256 match the saved source; otherwise every non-rejected range/item decision returns to proposed status for re-review and prior derivative receipts are cleared. Private mode keeps the register in the current tab only.

For an admitted source, the beta `pdf-lib` browser Web Worker can rebuild one reviewed page range or up to 200 selected confirmed ranges into research derivatives. Batch preflight reports invalid or out-of-bounds ranges, exact duplicates, overlaps, and uncovered source gaps; duplicate or overlap warnings require explicit acknowledgement. The processor checks the reviewed source SHA-256 before parsing, checks every selected source page before producing output, and refuses the whole export if any selected page has a non-empty `/Annots` array. For annotation-free pages it removes `/AA` additional-action dictionaries and empty annotation arrays. A batch ZIP includes ordered PDFs, `manifest.json`, `register.csv`, `README.md`, and `SHA256SUMS.txt`; described-only items remain manifest-only. Processing is cancellable and limited to five minutes and 200 MiB of derivative output. The rebuilt files are not byte-identical to the official source and are not official source files. Sources above 100 MiB are unsupported by the Packet Lab. Official source records and agency determinations control.
...
- Reopening a packet register saved before version 1.3 removes legacy pattern-generated suggestions and resets retained non-rejected items for an annotation-aware rescan and human review; the official source is never changed.
- Packet opening transfers the complete source through Cloudflare into browser memory and is hard-limited to 100 MiB. Beta derivative export obtains one fresh complete copy per single or batch operation, requires a matching source SHA-256, is limited separately to three full-source derivative requests per minute, and allows five minutes of browser-worker processing. Annotation-bearing pages are blocked rather than stripped; annotation-free copied pages have additional actions and empty annotation arrays removed, so every derivative is intentionally not byte-identical. Sources above 100 MiB are unsupported.
